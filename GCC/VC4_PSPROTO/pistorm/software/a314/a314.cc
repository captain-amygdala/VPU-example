...
#define logger_trace(...)                                          \
  do {                                                             \
    if (LOGGER_TRACE >= LOGGER_SHOW) fprintf(stdout, __VA_ARGS__); \
  } while (0)
#define logger_debug(...)                                          \
  do {                                                             \
    if (LOGGER_DEBUG >= LOGGER_SHOW) fprintf(stdout, __VA_ARGS__); \
  } while (0)
#define logger_info(...)                                          \
  do {                                                            \
    if (LOGGER_INFO >= LOGGER_SHOW) fprintf(stdout, __VA_ARGS__); \
  } while (0)
#define logger_warn(...)                                          \
  do {                                                            \
    if (LOGGER_WARN >= LOGGER_SHOW) fprintf(stdout, __VA_ARGS__); \
  } while (0)
#define logger_error(...)                                          \
  do {                                                             \
    if (LOGGER_ERROR >= LOGGER_SHOW) fprintf(stderr, __VA_ARGS__); \
  } while (0)
...
static void handle_msg_read_mem_req(ClientConnection *cc) {
  ...
  create_and_send_msg(cc, MSG_READ_MEM_RES, 0, &fastmem[address], length);
}
...
static void handle_msg_write_mem_req(ClientConnection *cc) {
  ...
  memcpy(&fastmem[address], &(cc->payload[4]), length);
  ...
  for (auto &on_demand : on_demand_services) {
    if (on_demand.service_name == service_name) {
      ...
      if (child == -1) {
        ...
        // FIXME: The user should be configurable.
        if (setgid(1000) < 0) {
          logger_error("Unable to set gid=1000 (group pi), errno=%d\n", errno);
        }
        if (setuid(1000) < 0) {
          logger_error("Unable to set uid=1000 (user pi), errno=%d\n", errno);
        }
        char home_env[] = "HOME=/home/pi";
        putenv(home_env);
...
static unsigned int a314_read_memory_8(unsigned int address) {
  address -= a314_base;
...
static unsigned int a314_read_memory_16(unsigned int address) {
  address -= a314_base;
  if (address >= sizeof(ca))
    return 0;
...
static unsigned int a314_read_memory_32(unsigned int address) {
  address -= a314_base;
  if (address >= sizeof(ca))
    return 0;
...
static void a314_write_memory_8(unsigned int address, unsigned int value) {
  address -= a314_base;
  if (address >= sizeof(ca))
    return;
...
static void a314_write_memory_16(unsigned int address, unsigned int value) {
  // Not implemented.
}

static void a314_write_memory_32(unsigned int address, unsigned int value) {
  // Not implemented.
}
...
static void ac_done_callback(int configured, unsigned int base) {
  if (!configured) {
    logger_error("Error: autoconfig did not configure a314\n");
    return;
  }

  a314_base = base;

  ca.mem_base = htobe32(FASTMEM_BASE);
  ca.mem_size = htobe32(FASTMEM_SIZE);

  struct ps_device device = {
      a314_read_memory_8, a314_read_memory_16, a314_read_memory_32,
      a314_write_memory_8, a314_write_memory_16, a314_write_memory_32};
...
int a314_init() {
  ...
  add_autoconfig_pic(ac_rom, sizeof(ac_rom), ac_done_callback);
  return 0;
}
